...
# 监控下单方法
watch cn.iocoder.boot.hmdianping.controller.voucher.VoucherOrderController createSeckillVoucherOrder '{params,returnObj,#cost}' -x 2 -n 20

# 监控服务层方法
watch cn.iocoder.boot.hmdianping.service.voucher.Impl.VoucherOrderServiceImpl seckillVoucherLua '{params,returnObj,#cost}' -x 2 -n 20

# 监控ID生成器
watch cn.iocoder.boot.framework.redis.core.RedisIdWorker nextId '{params,returnObj,#cost}' -x 2 -n 20
...
echo "第三步：系统资源监控"
echo "------------------------------------------"
echo "1. CPU使用率：top 或 htop"
echo "2. 内存使用：free -h"
echo "3. 网络连接：netstat -an | grep -E '(9876|6379)'"
echo "4. 磁盘IO：iostat -x 1"
echo ""
